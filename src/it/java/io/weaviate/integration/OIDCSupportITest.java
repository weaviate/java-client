...
public class OIDCSupportITest extends ConcurrentTest {
  ...
  /**
   * SpyTokenProvider is an Authorization implementation that spies on the
   * TokenProvider it creates and can expose tokens generated by it.
   */
  private static class SpyTokenProvider implements Authorization, TokenProvider {

    /** Exchange resource owner password for a token and return it. */
    static Token stealToken() throws Exception {
      var authz = Authorization.resourceOwnerPassword(WCS_DUMMY_CI_USERNAME, WCS_DUMMY_CI_PW, List.of());
      var spy = new SpyTokenProvider(authz);
    ...
    private SpyTokenProvider(Authorization actual) {
