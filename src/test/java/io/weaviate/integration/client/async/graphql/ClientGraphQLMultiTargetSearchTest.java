...
public class ClientGraphQLMultiTargetSearchTest extends AbstractAsyncClientTest {
  ...
  public void shouldPerformMultiTargetSearch() throws InterruptedException {
    ...
    response = doGet(
        get -> get.withClassName(className).withNearVector(nearVector1).withFields(_additional));
    ...
    response = doGet(
        get -> get.withClassName(className).withNearVector(nearVector2).withFields(_additional));
    assertNull("check error in response:", response.getError());
    assertGetContainsIds(response, className, id2, id3);
    // nearObject
    targets = Targets
  ...
  private void setupDB(String className) {
    ...
    List<Property> properties = Arrays.asList(
        Property.builder().name("title").dataType(Collections.singletonList(DataType.TEXT)).build(),
        Property.builder().name("content").dataType(Collections.singletonList(DataType.TEXT))
            .build(),
  ...
  private WeaviateClass.VectorConfig getFlatBQVectorConfig(Map<String, Object> vectorizerConfig) {
    return WeaviateClass.VectorConfig.builder().vectorIndexType("flat").vectorizer(vectorizerConfig)
        .vectorIndexConfig(
  ...
  private WeaviateClass.VectorConfig getHNSWVectorConfig(Map<String, Object> vectorizerConfig) {
    return WeaviateClass.VectorConfig.builder().vectorIndexType("hnsw").vectorizer(vectorizerConfig)
        .build();
  }
  ...
  private WeaviateClass.VectorConfig getHNSWPQVectorConfig(Map<String, Object> vectorizerConfig) {
    return WeaviateClass.VectorConfig.builder().vectorIndexType("hnsw").vectorizer(vectorizerConfig)
        .vectorIndexConfig(
            VectorIndexConfig.builder().pq(PQConfig.builder().enabled(true).build()).build())
        .build();
  }
  ...
  private void assertGetContainsIds(Result<GraphQLResponse> response, String className,
      String... expectedIds) {
    assertThat(response).isNotNull().returns(false, Result::hasErrors).extracting(Result::getResult)
        .isNotNull().extracting(GraphQLResponse::getData).isInstanceOf(Map.class)
        .extracting(data -> ((Map<String, Object>) data).get("Get")).isInstanceOf(Map.class)
        .extracting(get -> ((Map<String, Object>) get).get(className)).isInstanceOf(List.class)
        .asList().hasSize(expectedIds.length)
