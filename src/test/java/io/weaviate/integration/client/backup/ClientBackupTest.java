...
public class ClientBackupTest {
  ...
  public void shouldCreateAndRestoreBackupWithWaiting() {
    ...
    Supplier<Result<BackupRestoreResponse>> supplierRestoreResult = () -> client.backup().restorer()
        ...
        .withWaitForCompletion(true)
        .run();
    Supplier<Result<BackupRestoreStatusResponse>> supplierRestoreStatusResult = () -> client.backup()
        .restoreStatusGetter()
        .withBackend(BackupTestSuite.BACKEND)
        .withBackupId(backupId)
        .run();
  ...
  public void shouldListCreatedBackups() {
    ...
    Supplier<Result<BackupCreateResponse[]>> supplierGetResult = () -> client.backup().getter()
        .withBackend(BackupTestSuite.BACKEND).run();
  ...
  public void shouldFailOnRestoreBackupStatusOfNotStartedRestore() {
    ...
    BackupTestSuite.testFailOnRestoreBackupStatusOfNotStartedRestore(supplierCreateResult, supplierRestoreStatusResult,
        backupId);
  }
  ...
  public void shouldRestoreWithRbacOptions() {
    ...
    BackupTestSuite.testBackupRestoreWithRbacOptions(backupId,
        ...
        () -> {
          ...
          client.schema().classCreator().withClass(WeaviateClass.builder().className(className).build()).run();

          client.roles().deleter().withName(roleName).run();
          Result<?> createRole = client.roles().creator().withName(roleName)
              .withPermissions(Permission.cluster(ClusterPermission.Action.READ)).run();
          Assertions.assertThat(createRole.getError()).as("create role").isNull();

          client.users().db().deleter().withUserId(userName).run();
          Result<?> createUser = client.users().db().creator().withUserId(userName).run();
